  ...
  return (
    <section className="container-site py-16">
      ...
      {loading ? (
        ...
        <p className="mt-12 text-center text-forest-dark/60 dark:text-sand-dark">{t('blog.list.empty')}</p>
      ) : (
        <>
          {featured && (
            <Link
              to={`/blog/${featured.slug}/`}
              className="group mt-12 grid overflow-hidden rounded-3xl bg-forest-darker shadow-soft transition hover:shadow-soft-lg lg:grid-cols-2"
            >
            ...
            <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {rest.map((post) => (
                <Link
                  key={post.id}
                  to={`/blog/${post.slug}/`}
                  className="card group flex flex-col overflow-hidden transition duration-300 hover:-translate-y-1.5 hover:shadow-soft"
                >
